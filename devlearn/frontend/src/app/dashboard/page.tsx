...
export default function DashboardPage() {
  ...
  if (isLoading) return (
    <>
      ...
      <div className="max-w-5xl mx-auto px-6 py-12 space-y-4">
        {Array.from({ length: 4 }).map((_, i) => <div key={i} className="h-24 bg-card rounded-[12px] animate-pulse" />)}
      </div>
    </>
  );
  ...
  const avgProgress = enrollments.length
    ? Math.round(enrollments.reduce((s: number, e: any) => s + e.progressPct, 0) / enrollments.length)
    : 0;
  ...
  return (
    <>
      ...
      <main className="max-w-5xl mx-auto px-6 py-10">
        ...
        <div className="grid grid-cols-3 gap-3 mb-8">
          {[
            ...
            <div key={k.label} className="bg-card border border-border rounded-[12px] p-4">
              <div className="font-mono text-[26px] font-medium mt-0.5" style={{ color: k.color as any }}>{k.value}</div>
        ...
        {/* My courses */}
        <h2 className="font-mono text-[11px] text-[#64748b] uppercase tracking-[0.5px] mb-4">Khóa học của tôi</h2>
                ...
                <div key={c.id} className="bg-card border border-border rounded-[12px] p-5">
                  <div className="flex items-start gap-3 mb-4">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="font-semibold text-[14px] leading-snug truncate">{c.title}</p>
                  ...
                  <div className="mb-1 flex justify-between text-[11px] text-[#64748b]">
                    <span>Tiến độ bài học</span><span>{pct}%</span>
                  </div>
                  ...
                  {en.lastQuizAttempt && (
                    <div className="mb-4 flex justify-between text-[11px] text-[#64748b]">
                      ...
                      <span className={passed ? 'text-acc3' : 'text-yellow-400'}>{passed ? '✓ Đạt' : '✗ Chưa đạt'}</span>
                    </div>
                  )}

                  <div className="flex gap-2">
                    <Link href={`/courses/${c.slug}`} className="btn-primary flex-1 text-center text-[12px] py-2">▶ Học tiếp</Link>
                    <Link href={`/courses/${c.slug}/quiz`} className="btn-secondary text-[12px] py-2 px-3">📝 Quiz</Link>
                    {en.certificate && (
                      <Link href={`/certificate/${en.certificate.code}`} className="btn-secondary text-[12px] py-2 px-3">🏆</Link>
                    )}
                  </div>
                </div>
              );
