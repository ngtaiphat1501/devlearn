      ...
      <div className="max-w-6xl mx-auto px-6 py-8">
        ...
        {isLoading ? (
          ...
          <div className="text-center py-16 text-[#64748b]">
            ...
            <p className="mb-4">Chưa có khóa học nào.</p>
            <Link href="/admin/courses/new" className="btn-primary">+ Tạo khóa học đầu tiên</Link>
          </div>
        ) : (
              ...
              <thead>
                <tr className="border-b border-border">
                  {['Khóa học', 'Danh mục', 'Level', 'Giá', 'Học viên', 'Bài học', 'Trạng thái', 'Thao tác'].map(h => (
                    <th key={h} className="text-left px-4 py-3 text-[11px] font-semibold text-[#64748b] uppercase tracking-[0.5px] font-mono bg-[rgba(0,0,0,0.2)]">
                      {h}
                    </th>
                  ))}
                </tr>
              ...
              <tbody>
                {data?.data?.map((c: any) => (
                  <tr key={c.id} className="border-b border-border hover:bg-[rgba(255,255,255,0.015)]">
                    ...
                    <td className="px-4 py-3">
                      <span className={`badge ${LEVEL_CLASS[c.level] ?? 'badge-blue'}`}>
                    ...
                    <td className="px-4 py-3">
                      <span className="flex items-center gap-1 text-[#94a3b8]">
                        <BookOpen size={12} />{c.totalLessons ?? 0}
                      </span>
                    </td>
                    <td className="px-4 py-3">
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-1.5">
                        ...
                        <button
                          ...
                          className="p-1.5 rounded-[6px] bg-[rgba(239,68,68,0.08)] text-red-400 hover:bg-[rgba(239,68,68,0.18)] transition-colors disabled:opacity-50"
                          title="Xóa khóa học"
                        >
                          <Trash2 size={13} />
                        </button>
